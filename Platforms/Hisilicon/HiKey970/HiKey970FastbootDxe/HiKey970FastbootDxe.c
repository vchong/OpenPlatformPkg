...
/*
  ...
  HiKey970FastbootPlatformFlashPartition.
*/
EFI_STATUS
HiKey970FastbootPlatformInit (
  VOID
  )
{
  EFI_STATUS               Status;
...
VOID
HiKey970FastbootPlatformUnInit (
  VOID
  )
{
  FreePartitionList ();
  ...
  )
{
  ...
  if (AsciiStrCmp (PartitionName, "ptable") == 0) {
    return HiKey970FlashPtable (Size, Image);
  } else if (AsciiStrCmp (PartitionName, "xloader") == 0) {
    return HiKey970FlashXloader (Size, Image);
  }
...
EFI_STATUS
HiKey970FastbootPlatformErasePartition (
  IN CHAR8 *PartitionName
  )
{
  EFI_STATUS                  Status;
...
EFI_STATUS
HiKey970FastbootPlatformGetVar (
  IN  CHAR8   *Name,
  OUT CHAR8   *Value
  )
{
  ...
  )
{
  ...
  if (AsciiStrCmp (Command, "Demonstrate") == 0) {
    ...
    DEBUG ((DEBUG_ERROR,
      "HiKey970: Unrecognised Fastboot OEM command: %a\n",
      Command
      ));
    return EFI_NOT_FOUND;
  }
...
EFI_STATUS
HiKey970FastbootPlatformFlashPartitionEx (
  IN CHAR8  *PartitionName,
  IN UINTN   Offset,
  IN UINTN   Size,
  IN VOID   *Image
...
EFI_STATUS
EFIAPI
HiKey970FastbootPlatformEntryPoint (
