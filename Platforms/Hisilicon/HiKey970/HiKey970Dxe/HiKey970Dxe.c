...
#include <Guid/HiKey970Variable.h>
...
#define HIKEY970_COMPATIBLE_HUB_V1       "hisilicon,gpio_hubv1"
#define HIKEY970_COMPATIBLE_HUB_V2       "hisilicon,gpio_hubv2"
...
#define DETECT_SW_FASTBOOT               55        // GPIO6_7
  ...
  )
{
  ...
  // GPIO205
  MmioWrite32 (0xfff11000 + (27 << 2), 0); // GPIO function
  ...
  )
{
  ...
  UnicodeSPrint (
    ...
    L" earlycon=pl011,0xfff32000,115200 console=ttyAMA6 androidboot.serialno=%s clk_ignore_unused=true",
    RandomSN->UnicodeSN
    );
  DEBUG ((DEBUG_ERROR, "cmdline:%s\n",Args));
  FreePages (DataPtr, 1);
  return EFI_SUCCESS;
Exit:
  FreePages (DataPtr, 1);
  ...
  )
{
  ...
  if (mBoardId == HIKEY970_BOARDID_V1) {
    offset = fdt_node_offset_by_compatible (
               (VOID*)(UINTN)*NewFdtBase, -1, HIKEY970_COMPATIBLE_LEDS_V1
               );
  } else {
    offset = fdt_node_offset_by_compatible (
               (VOID*)(UINTN)*NewFdtBase, -1, HIKEY970_COMPATIBLE_LEDS_V2
               );
  }
  ...
  if (mBoardId == HIKEY970_BOARDID_V1) {
    offset = fdt_node_offset_by_compatible (
               (VOID*)(UINTN)*NewFdtBase, -1, HIKEY970_COMPATIBLE_HUB_V1
               );
  } else {
    offset = fdt_node_offset_by_compatible (
               (VOID*)(UINTN)*NewFdtBase, -1, HIKEY970_COMPATIBLE_HUB_V2
               );
  }
  // Ignore it if can't find LED compatible
  if (offset < 0) {
  ...
  )
{
  ...
  /* Configure GPIO55 as GPIO function */
  MmioWrite32 (0xe896c0a0, 0);
